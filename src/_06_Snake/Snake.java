...
public class Snake {
	...
	public void feed() {
		...
		Location location = snake.get(snake.size()-1).getLocation();
		SnakeSegment s = new SnakeSegment(location, BODY_SIZE);
		snake.add(s);
	...
	public boolean isOutOfBounds() {
		...
		if(head.getLocation().x<0) {
			...
			System.out.println(_00_SnakeGame.WINDOW_WIDTH);
			return true;
		}else if(head.getLocation().y >= _00_SnakeGame.HEIGHT) {
			System.out.println(_00_SnakeGame.WINDOW_HEIGHT);
			return true;
		}else{
			return false;
		}
	...
	public boolean isHeadCollidingWithBody() {
		...
		for(int i = 1; i < snake.size(); i++) {
			...
			if(head.getLocation().x == snake.get(i).getLocation().x) {
				if(head.getLocation().y == snake.get(i).getLocation().y) {
					return true;
				}
			}
		}
